...
try:
    from facebook.facebook_fetcher import normalize_topic_key, now_iso
    from utils.db_utils import DatasetDB
except ModuleNotFoundError:
    from backend.scripts.datasets.facebook.facebook_fetcher import normalize_topic_key, now_iso
...
class FacebookDatasetWriter:
    ...
    def write_payload(
        ...
        collected_at = _normalize_string(meta.get("collected_at")) or now_iso()
        topic_rows_meta = meta.get("topic_rows") if isinstance(meta.get("topic_rows"), list) else []
        ...
        inserted_topics = inserted_users = inserted_contents = inserted_content_topics = inserted_user_topics = 0
        ...
        return {
            ...
            "imported": {
                ...
                "contents": inserted_contents,
                "content_topics": inserted_content_topics,
                "user_topics": inserted_user_topics,
...
def import_facebook_payload_to_sqlite(
    payload: dict[str, Any],
    fetch_options: dict[str, Any],
    db_path: Path,
    platform: str = PLATFORM,
) -> dict[str, Any]:
    return FacebookDatasetWriter(db_path).write_payload(payload, fetch_options, platform)
