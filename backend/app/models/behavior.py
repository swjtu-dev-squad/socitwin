...
class ActionProbability(BaseModel):
    ...
    action_type: OASISActionType = Field(..., description="Type of action")
    probability: float = Field(default=0.0, ge=0.0, le=1.0, description="Probability weight (0-1)")
    conditions: Optional[Dict[str, Any]] = Field(
        default=None, description="Optional conditions for when this probability applies"
    )
    action_args: Dict[str, Any] = Field(
        default_factory=dict, description="Arguments to pass when executing this action"
    )
    description: Optional[str] = Field(
        default=None, description="Human-readable description of this probability rule"
    )
...
class ProbabilityDistribution(BaseModel):
    ...
    name: str = Field(..., description="Name of this probability distribution profile")
    description: Optional[str] = Field(default=None, description="Description of this distribution")
    actions: List[ActionProbability] = Field(
        default_factory=list, description="List of action probabilities"
    )
    platform: Optional[PlatformType] = Field(
        default=None, description="Platform this distribution is designed for"
    )
...
class RuleCondition(BaseModel):
    ...
    value: Optional[Any] = Field(
        default=None, description="Value to compare against (not needed for EXISTS/NOT_EXISTS)"
    )
...
class BehaviorRule(BaseModel):
    ...
    priority: int = Field(
        default=1, ge=1, le=10, description="Rule priority (higher = more important)"
    )
    conditions: List[RuleCondition] = Field(
        default_factory=list, description="List of conditions that must all be true"
    )
    action: OASISActionType = Field(..., description="Action to execute when rule triggers")
    action_args: Dict[str, Any] = Field(
...
class RuleSet(BaseModel):
    ...
    description: Optional[str] = Field(default=None, description="Description of this rule set")
    rules: List[BehaviorRule] = Field(default_factory=list, description="Rules in this set")
    platform: Optional[PlatformType] = Field(
        default=None, description="Platform this rule set is designed for"
    )
...
class TimelineEvent(BaseModel):
    ...
    step: int = Field(..., ge=0, description="Simulation step when this event occurs")
    action: OASISActionType = Field(..., description="Action to execute")
    action_args: Dict[str, Any] = Field(
        default_factory=dict, description="Arguments for the action"
    )
    repeat_interval: Optional[int] = Field(
        default=None, ge=1, description="Repeat interval in steps (None for one-time event)"
    )
    repeat_count: Optional[int] = Field(
        default=None, ge=1, description="Number of times to repeat (None for infinite)"
    )
...
class BehaviorSchedule(BaseModel):
    ...
    loop: bool = Field(default=False, description="Whether to loop the schedule after completion")
...
class StrategyWeight(BaseModel):
    ...
    strategy: BehaviorStrategy = Field(..., description="Strategy type")
    weight: float = Field(default=0.0, ge=0.0, le=1.0, description="Weight for this strategy (0-1)")
...
class AgentBehaviorConfig(BaseModel):
    ...
    strategy: BehaviorStrategy = Field(
        default=BehaviorStrategy.LLM_AUTONOMOUS, description="Primary behavior strategy"
    )

    # Strategy-specific configurations
    probability_distribution: Optional[ProbabilityDistribution] = Field(
        default=None, description="Probability distribution for probabilistic strategy"
    )
    rule_set: Optional[RuleSet] = Field(
        default=None, description="Rule set for rule-based strategy"
    )
    schedule: Optional[BehaviorSchedule] = Field(
        default=None, description="Schedule for scheduled strategy"
    )
    mixed_strategy: Optional[MixedStrategyConfig] = Field(
        default=None, description="Configuration for mixed strategy"
    )

    # Context and constraints
    enabled: bool = Field(
        default=True, description="Whether this behavior configuration is enabled"
    )
    platform_filter: Optional[PlatformType] = Field(
        default=None, description="Platform this configuration applies to"
    )
    step_range: Optional[tuple[int, int]] = Field(
        default=None, description="Step range where this configuration is active (inclusive)"
    )
    conditions: Optional[Dict[str, Any]] = Field(
        default=None, description="Additional conditions for when this configuration applies"
    )
...
class BehaviorProfile(BaseModel):
    ...
    behavior_config: AgentBehaviorConfig = Field(..., description="Behavior configuration")
...
class BehaviorContext(BaseModel):
    ...
    platform: PlatformType = Field(..., description="Current platform")
    agent_id: int = Field(..., description="Agent ID")
    agent_state: Optional[Dict[str, Any]] = Field(default=None, description="Current agent state")
    simulation_state: Optional[Dict[str, Any]] = Field(
        default=None, description="Overall simulation state"
    )
    recent_actions: List[Dict[str, Any]] = Field(
        default_factory=list, description="Recent actions taken by this agent"
    )
    environment_stats: Optional[Dict[str, Any]] = Field(
        default=None, description="Environment statistics"
    )
...
class BehaviorConfigRequest(BaseModel):
    ...
    agent_id: int = Field(..., description="Agent ID to configure")
    behavior_config: AgentBehaviorConfig = Field(..., description="Behavior configuration")
...
class BehaviorProfileRequest(BaseModel):
    ...
    profile: BehaviorProfile = Field(..., description="Behavior profile")
...
class ApplyProfileRequest(BaseModel):
    ...
    agent_ids: List[int] = Field(
        default_factory=list, description="List of agent IDs to apply to (empty = all agents)"
    )
...
class BehaviorConfigResponse(BaseModel):
    ...
    agent_id: Optional[int] = Field(default=None, description="Agent ID (if applicable)")
    profile_id: Optional[str] = Field(default=None, description="Profile ID (if applicable)")
...
class BehaviorProfilesResponse(BaseModel):
    ...
    profiles: List[BehaviorProfile] = Field(
        default_factory=list, description="List of behavior profiles"
    )
    count: int = Field(default=0, description="Total number of profiles")
    timestamp: datetime = Field(default_factory=datetime.now, description="Response timestamp")
...
def create_default_behavior_config() -> AgentBehaviorConfig:
    ...
    return AgentBehaviorConfig(strategy=BehaviorStrategy.LLM_AUTONOMOUS, enabled=True)


def create_probabilistic_config(
    name: str = "balanced", platform: PlatformType = PlatformType.TWITTER
) -> AgentBehaviorConfig:
    ...
    if platform == PlatformType.TWITTER:
        actions = [
            ...
            ActionProbability(
                action_type=OASISActionType.LIKE_POST, probability=0.3, description="Like a post"
            ),
            ActionProbability(
            ...
            ActionProbability(
                action_type=OASISActionType.REFRESH, probability=0.15, description="Refresh feed"
            ),
            ActionProbability(
                action_type=OASISActionType.DO_NOTHING, probability=0.1, description="Do nothing"
            ),
        ]
    else:
        # Default for Reddit
        actions = [
            ...
            ActionProbability(
                action_type=OASISActionType.LIKE_POST, probability=0.35, description="Upvote a post"
            ),
            ActionProbability(
                action_type=OASISActionType.CREATE_COMMENT,
                probability=0.3,
            ...
            ActionProbability(
                action_type=OASISActionType.REFRESH, probability=0.1, description="Refresh feed"
            ),
            ActionProbability(
            ...
            ActionProbability(
                action_type=OASISActionType.DO_NOTHING, probability=0.05, description="Do nothing"
            ),
        ]
    ...
    return AgentBehaviorConfig(
        strategy=BehaviorStrategy.PROBABILISTIC, probability_distribution=distribution, enabled=True
    )


def create_scheduled_config(
    name: str = "marketing_campaign", platform: PlatformType = PlatformType.TWITTER
) -> AgentBehaviorConfig:
    ...
    return AgentBehaviorConfig(strategy=BehaviorStrategy.SCHEDULED, schedule=schedule, enabled=True)


def create_rule_based_config(
    name: str = "customer_support", platform: PlatformType = PlatformType.TWITTER
) -> AgentBehaviorConfig:
    ...
    rules = [
        BehaviorRule(
            ...
            conditions=[
                RuleCondition(
                    field="event_type", operator=ConditionOperator.EQUALS, value="mentioned"
                ),
                RuleCondition(
                    field="sentiment", operator=ConditionOperator.EQUALS, value="positive"
                ),
            ],
            action=OASISActionType.CREATE_COMMENT,
        ...
        BehaviorRule(
            ...
            conditions=[
                ...
                RuleCondition(
                    field="sentiment", operator=ConditionOperator.EQUALS, value="negative"
                ),
            ],
            action=OASISActionType.CREATE_COMMENT,
            action_args={
                "content": "We're sorry to hear about your experience. Please DM us so we can help."
            },
        ),
        BehaviorRule(
            ...
            conditions=[
                ...
                RuleCondition(
                    field="sentiment", operator=ConditionOperator.EQUALS, value="very_positive"
                ),
                RuleCondition(
                    field="influence", operator=ConditionOperator.GREATER_THAN, value=1000
                ),
            ],
    ...
    return AgentBehaviorConfig(
        strategy=BehaviorStrategy.RULE_BASED, rule_set=rule_set, enabled=True
    )
