...
class HeuristicUnicodeTokenCounter(BaseTokenCounter):
    """Offline-safe fallback token counter.

    The goal is not provider-level accuracy. The goal is a conservative and
    stable approximation that can support budget checks and CAMEL chunking.
    """
    ...
    def _estimate_value(self, value: Any) -> int:
        ...
        if isinstance(value, dict):
            return sum(
                self._estimate_text_tokens(str(key))
                + self._estimate_value(item)
