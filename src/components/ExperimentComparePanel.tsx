...
import { listExperiments, getExperimentResult, ExperimentListItem, ExperimentRunResult } from '../lib/experimentApi';
import { extractCompareMetrics, buildCompareResult, CompareMetrics } from '../lib/compareApi';
...
export default function ExperimentComparePanel() {
  ...
  useEffect(() => {
    if (!expA) return;
    getExperimentResult(expA).then(r => {
      setResultA(r);
  ...
  useEffect(() => {
    if (!expB) return;
    getExperimentResult(expB).then(r => {
      setResultB(r);
      const firstRec = r.runs?.[0]?.recommender || '';
      setRunB(firstRec);
  ...
  const barData = compare ? [
    { name: '极化指数', A: compare.runA.polarization_final, B: compare.runB.polarization_final },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={fetchExperiments} className="flex items-center gap-1 text-xs text-gray-400 hover:text-white transition-colors">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-gray-800 rounded-xl p-4 border-l-4 border-blue-400">
          ...
          >
            ...
            {experiments.map(e => (
              <option key={e.experimentId} value={e.experimentId}>{e.name}</option>
            ))}
          </select>
          {resultA && (
            <select
            ...
            >
              {resultA.runs?.map(r => (
                <option key={r.recommender} value={r.recommender}>{r.recommender}</option>
              ))}
            </select>
          )}
        </div>
      ...
      {compare && (
        <>
          ...
          <div className="grid grid-cols-4 gap-3">
            <DiffCard label="极化差异" a={compare.runA.polarization_final} b={compare.runB.polarization_final} />
            <DiffCard label="羊群差异" a={compare.runA.herd_index_final} b={compare.runB.herd_index_final} />
            <DiffCard label="速度差异" a={compare.runA.velocity_avg} b={compare.runB.velocity_avg} />
            <DiffCard label="帖子差异" a={compare.runA.total_posts} b={compare.runB.total_posts} integer />
          </div>
          ...
          <div className="bg-gray-800 rounded-xl p-4">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={traceData}>
                ...
                <Legend />
                <Line type="monotone" dataKey="A_pol" name={`A: ${metricsA?.recommender}`} stroke={COLORS.A} dot={false} strokeWidth={2} />
                <Line type="monotone" dataKey="B_pol" name={`B: ${metricsB?.recommender}`} stroke={COLORS.B} dot={false} strokeWidth={2} />
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="bg-gray-800 rounded-xl p-4">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={traceData}>
                ...
                <Tooltip contentStyle={{ background: '#1f2937', border: '1px solid #374151', borderRadius: 8 }} />
                <Legend />
                <Line type="monotone" dataKey="A_herd" name={`A: ${metricsA?.recommender}`} stroke={COLORS.A} dot={false} strokeWidth={2} />
                <Line type="monotone" dataKey="B_herd" name={`B: ${metricsB?.recommender}`} stroke={COLORS.B} dot={false} strokeWidth={2} />
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="bg-gray-800 rounded-xl p-4">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={barData}>
                ...
                <Tooltip contentStyle={{ background: '#1f2937', border: '1px solid #374151', borderRadius: 8 }} />
                <Legend />
                <Bar dataKey="A" name={`A: ${metricsA?.recommender}`} fill={COLORS.A} radius={[4, 4, 0, 0]} />
                <Bar dataKey="B" name={`B: ${metricsB?.recommender}`} fill={COLORS.B} radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="bg-gray-800 rounded-xl p-4">
            ...
            <ResponsiveContainer width="100%" height={240}>
              <RadarChart data={radarData}>
                ...
                <Radar name={`A: ${metricsA?.recommender}`} dataKey="A" stroke={COLORS.A} fill={COLORS.A} fillOpacity={0.2} />
                <Radar name={`B: ${metricsB?.recommender}`} dataKey="B" stroke={COLORS.B} fill={COLORS.B} fillOpacity={0.2} />
                <Legend />
                <Tooltip contentStyle={{ background: '#1f2937', border: '1px solid #374151', borderRadius: 8 }} />
              </RadarChart>
            </ResponsiveContainer>
...
function DiffCard({ label, a, b, integer }: { label: string; a: number; b: number; integer?: boolean }) {
  const diff = b - a;
  const isPos = diff > 0;
