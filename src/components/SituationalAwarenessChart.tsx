...
export const SituationalAwarenessChart = () => {
  ...
  // 1. 准备雷达图数据（当前快照）
  // 传播速度需要归一化处理（假设 10 msg/s 为 1.0 的参考上限）
  const radarData = useMemo(() => {
    const normVelocity = Math.min((status.velocity || 0) / 10, 1);
    return [
      { subject: '群体极化 (Polarization)', value: status.polarization || 0, fullMark: 1 },
      { subject: '信息传播 (Propagation)', value: normVelocity, fullMark: 1 },
  ...
  const trendData = useMemo(() => {
    return history.map((h) => ({
      ...
      极化: h.polarization,
      从众: h.herdHhi || 0,
      传播: Math.min((h.velocity || 0) / 10, 1), // 归一化显示
    }));
  }, [history]);
      ...
      <Card className="lg:col-span-1 p-6 bg-bg-secondary border-border-default flex flex-col items-center justify-center relative overflow-hidden">
        ...
        <div className="flex gap-4 mt-2">
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-lg font-mono text-accent">{(status.velocity || 0).toFixed(1)}</span>
          </div>
        </div>
      </Card>
