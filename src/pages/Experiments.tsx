...
import { PlatformStrategy, InferenceTask } from '@/lib/labTypes';
...
const INITIAL_PLATFORMS: PlatformStrategy[] = [
  { id: 'tiktok', name: 'TikTok', params: { interestWeight: 0.8, socialWeight: 0.2, recencyWeight: 0.9, qualityWeight: 0.4, explorationRate: 0.7 } },
  { id: 'xiaohongshu', name: '小红书', params: { interestWeight: 0.6, socialWeight: 0.7, recencyWeight: 0.5, qualityWeight: 0.8, explorationRate: 0.4 } },
  { id: 'pinterest', name: 'Pinterest', params: { interestWeight: 0.9, socialWeight: 0.3, recencyWeight: 0.2, qualityWeight: 0.7, explorationRate: 0.6 } },
  { id: 'reddit', name: 'Reddit', params: { interestWeight: 0.5, socialWeight: 0.8, recencyWeight: 0.6, qualityWeight: 0.9, explorationRate: 0.3 } },
  { id: 'twitter', name: 'X/Twitter', params: { interestWeight: 0.4, socialWeight: 0.6, recencyWeight: 0.95, qualityWeight: 0.3, explorationRate: 0.8 } },
];
...
export default function Experiments() {
  ...
  const handleParamUpdate = (platformId: string, paramKey: keyof PlatformStrategy['params'], value: number) => {
          ...
          {tabs.map(tab => (
            <button
              ...
              className={cn(
                ...
                activeTab === tab.id ? "bg-accent text-bg-primary shadow-lg shadow-accent/10" : "text-text-tertiary hover:text-text-secondary"
              )}
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <Card className="lg:col-span-4 bg-bg-secondary border-border-default p-8 space-y-8 h-fit">
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-text-tertiary">实验名称</label>
                  <Input placeholder="例如: TikTok vs XHS 极化对比" className="bg-bg-primary border-border-default rounded-xl" />
                </div>

                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-text-tertiary">选择数据集</label>
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-text-tertiary">参照基准 (Baseline)</label>
                  <select className="w-full h-11 bg-bg-primary border border-border-default rounded-xl px-4 text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-accent/20">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-text-tertiary">对比策略</label>
                  <div className="grid grid-cols-1 gap-2">
                    {platforms.map(platform => (
                      <label key={platform.id} className="flex items-center gap-3 p-3 rounded-xl bg-bg-primary border border-border-default cursor-pointer hover:border-accent/30 transition-colors">
                        <input type="checkbox" className="w-4 h-4 accent-accent" defaultChecked={['tiktok', 'xiaohongshu'].includes(platform.id)} />
            ...
            <Card className="lg:col-span-8 bg-bg-secondary border-border-default flex flex-col overflow-hidden min-h-[500px]">
              <div className="p-4 border-b border-border-default bg-bg-secondary/50 flex justify-between items-center">
                <h2 className="text-sm font-bold uppercase tracking-widest text-text-tertiary">活跃任务队列与拟合监控</h2>
          ...
          <Card className="bg-bg-secondary border-border-default overflow-hidden">
            <div className="p-6 border-b border-border-default bg-bg-secondary/50 flex flex-wrap gap-4 items-center">
              <div className="relative flex-1 min-w-[300px]">
                ...
                <Input placeholder="搜索实验记录..." className="pl-10 h-11 rounded-xl bg-bg-primary border-border-default" />
              </div>
            ...
            <ScrollArea className="h-[600px]">
              <div className="divide-y divide-border-default/50">
                {[1, 2, 3, 4, 5].map(i => (
                  <div key={i} className="p-6 hover:bg-bg-tertiary/30 transition-all flex items-center justify-between group cursor-pointer">
                    ...
                    <div className="flex items-center gap-8">
                      <div className="text-right hidden md:block">
                        <p className="text-[10px] font-bold text-text-tertiary uppercase tracking-widest">Fit Score</p>
                        <p className="text-lg font-mono font-bold text-emerald-500">{80 + i * 2}%</p>
                      </div>
