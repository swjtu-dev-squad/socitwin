...
这一节不是新的检索指标，而是辅助解释本轮样本结构。

它回答两个问题：

1. 本轮模拟实际产生了哪些类型的长期记忆。
2. 帖子关联检索的“正确答案”主要来自哪些动作类型。

这部分对分析很重要，因为不同动作写入长期记忆后的文本强度不一样：

- `quote_post` 通常包含 agent 自己生成的自然语言内容，语义更丰富，容易被向量检索和 rerank 排到前面。
- `like_post` 和 `repost` 更像结构化互动，动作本身文本少，主要依赖目标帖子正文和结构字段。
- `create_post` 是 agent 自己发出的新帖子，不一定和某个 observation 中的已有帖子构成帖子关联正确答案。
- `follow` 是作者关系动作，不是帖子互动动作，所以不会进入本轮帖子关联检索的正确答案集合。

### S1 动作分布

| 动作        | 写入长期记忆数量 | 作为帖子关联正确答案出现次数 |
| ----------- | ---------------: | --------------------------: |
| like_post   |               27 |                         107 |
| quote_post  |               18 |                          85 |
| repost      |                7 |                          32 |
...
- “写入长期记忆数量”表示模拟中实际产生并存入长期记忆的 episode 数。
- “作为帖子关联正确答案出现次数”表示这些 episode 在帖子检索题目中被判定为正确答案的次数。同一条 episode 可能对应多个可见帖子 probe，所以这个数可以大于写入数量。
- S1 写入最多的是 `like_post`，但 `like_post` 的文本信号较弱。
- S1 正确答案里 `like_post` 和 `quote_post` 都很多，因此很容易出现“同一个 agent、同一个大话题下，语义更丰富的 quote 压过结构上正确的 like/repost”的情况。
- 这能解释为什么 S1 的同 agent 错目标数量更高。
...
- “写入长期记忆数量”和“作为帖子关联正确答案出现次数”的含义同 S1。
- S2 的 `create_post` 和 `quote_post` 占比更高，整体文本信号比纯点赞/转发更强。
- 本轮 S2 的帖子关联 Hit@3 更高，可能和可检索文本更丰富、错目标更少有关。
- 但 S2 只跑了一轮，不能直接据此判断该场景天然更容易。

统一说明：

- `follow` 会写入长期记忆，但不会作为 帖子关联检索 的硬正确答案。
- 原因是当前 query 是帖子摘要，不是作者关系问题。
- follow、mute、重复关注等关系记忆应该后续单独设计 author-based recall 测试。
