...
- 当前已实现的 `VAL-RCL-11` 属于 post-based runtime replay：query 来自真实 observation 中每个可见 post 的 `summary`，ground truth 来自结构化 post 关系。
...
- `B-level v0.5`：补 post-based runtime replay；
...
- runtime-query related retrieval
  ...
  - 当前已先实现 post-based runtime replay 版本；
...
- 后者第一版使用真实 observation 里的每个可见 post summary 出题，用来判断“agent 看到某个帖子时，能否找回自己过去和这个帖子结构相关的历史”。
...
当前 `VAL-RCL-11 post_based_runtime_replay` 的 ground truth 只接受结构化 post 关系：

- same agent；
- 历史 episode 早于当前 observation step；
...
- `S2 similar-topic interference pack`
  - 加强 post-based runtime replay、完整 query trace replay、same-agent hard negatives 和 cross-agent guardrail；
...
- 当前已补 `VAL-RCL-11 post_based_runtime_replay`，用于每个可见 post 的 observation-summary 检索；
...
4. 补 post-based runtime replay，使用真实 observation 里的每个 post summary 评估相关历史是否能被找回。
