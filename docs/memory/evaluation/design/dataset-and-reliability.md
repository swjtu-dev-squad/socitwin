...
- 当前已实现的 `VAL-RCL-11` 属于 post-linked final lookup：query 来自真实 observation 中每个可见 post 的 `summary`，ground truth 来自最终长期记忆中的结构化 post 关系。
- B 级后续仍需要补完整 `last_recall_query_text` trace replay、author-based relationship recall 和 group recall。
...
- 已有 agent count、step count、timeout、probe limit 等参数；其中 probe limit 默认 `0` 表示全量候选；
- 还没有 pack 级多次运行聚合；
- 还没有严格 usable probe validity gate；
- 已有 `VAL-RCL-11 post_linked_final_lookup`；
- 还没有完整 `last_recall_query_text` trace replay 指标。

因此：

- 当前代码里已经有 B-level v0 固定输入和 episode self-retrievability；
- 当前代码里已经有 B-level v0.5 post-linked final lookup；
- run/pack/overall 聚合仍属于目标设计；
- 完整 `last_recall_query_text` trace replay、author-based relationship recall 和 group recall 仍是后续缺口。

后续建议把 B 级拆成两步：

- `B-level v0`：固定输入、样本质量统计、episode self-retrievability；
- `B-level v0.5`：补 post-linked final lookup；
...
- runtime-query related retrieval
  ...
  - 当前已先实现 post-linked final lookup 版本；
  - ground truth 不宜强制 single exact episode，应基于 query source post/topic/target 构造 related episode set。

必须注意：当前 `episode self-retrievability` 和 `runtime-query related retrieval` 不是同一个测试。

- 前者使用 episode 自身字段构造 probe，用来判断“写入长期记忆的条目在有利线索下是否还能被找回”；
- 后者第一版使用真实 observation 里的每个可见 post summary 出题，用来判断“整场模拟结束后，能否从该 agent 的最终长期记忆中找回它和这个帖子结构相关的动作事件”。

如果 episode self-retrievability 好但 runtime-query related retrieval 差，优先说明 query 构造、query source、rerank 或相关性定义存在问题，而不是简单说明长期记忆写入失败。

2026-04-26 的 S1/S2 18-step 检查暴露了一个典型边界：

- `create_post` 和 `quote_post` 在 episode self-retrievability 中表现稳定；
- `like_post` 明显更弱，因为它的 action episode 自身语义稀薄，容易被同一 agent 的同主题 `quote_post` 或其他 richer episode 压过；
- cross-agent contamination 在该轮为 0，因此该问题更像 same-agent same-topic ranking ambiguity，而不是 agent filter 失效。

这说明 B 级测试不能只看一个总分。按 action type、miss examples、query 文本、top-k 候选和 selected/injected trace 拆开看，才有助于判断问题在写入、查询、排序、过滤还是注入。

当前 `VAL-RCL-11 post_linked_final_lookup` 的 ground truth 只接受最终长期记忆中的结构化 post 关系：

- same agent；
- 目标是同一个 `source_post_id`；
- 或评论 episode 的 `local_context.parent_post.post_id` 指向该 post；
- 或 self-authored 可见 post 对应历史 `create_post` episode。

检索文本只使用 post `summary`，不把 `post_id` 放入 query。`post_id` 只用于测试判题和调试。

因此 `VAL-RCL-11` 是 B 级里更贴近 runtime observation 的检索测试，但它仍不是完整行为测试，也不是关系记忆测试。`follow` / `mute` 等 user-oriented memory 需要后续单独设计 author-based recall。
...
- `S2 similar-topic interference pack`
  - 加强 post-linked final lookup、完整 query trace replay、same-agent hard negatives 和 cross-agent guardrail；
...
- 当前已补 `VAL-RCL-11 post_linked_final_lookup`，用于每个可见 post 的 observation-summary 检索；
- 后续缺口是更完整的 `last_recall_query_text` trace replay、author-based relationship recall 和 group recall；
...
4. 补 post-linked final lookup，使用真实 observation 里的每个 post summary 评估最终长期记忆中的相关 episode 是否能被找回。
