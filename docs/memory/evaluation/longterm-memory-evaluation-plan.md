...
因此 exact episode hit 仍有价值，但不能单独代表完整 runtime recall 质量。当前 B 级已先补 `VAL-RCL-11 post_linked_final_lookup`，用每个可见 post 的 `summary` 出题并用结构化 post 关系判定最终长期记忆中的正确答案；完整 `last_recall_query_text` trace replay 仍留作后续增强。
...
- B 级已补 `VAL-RCL-11 post_linked_final_lookup`：使用真实 observation 中每个可见 post 的 summary 出题，评估是否能在最终长期记忆中找回同 agent 和该 post 结构相关的 episode。
...
- 当前已用 `VAL-RCL-11` 先补 final-store post-linked lookup；
...
- runtime query 主要来自当前 observation summary；当前已用 `VAL-RCL-11` 先覆盖 final-store post-linked lookup，后续仍需要完整 `last_recall_query_text` trace replay 补测。
