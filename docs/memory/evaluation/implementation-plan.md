...
- Status: Phase 1, B-level v0 fixed replay, and B-level v0.5 post-based runtime replay implemented
...
- B-level v0.5
  ...
  - 它评估 post-linked runtime observation recall，不把 `post_id` 放入检索文本。
- B-level 缺口
...
Status: implemented for fixed-input self-retrievability; post-based runtime replay implemented; full query trace replay pending.
...
## 5. Phase 4: Post-Based Runtime Replay

Status: implemented as `VAL-RCL-11 post_based_runtime_replay`.
...
- 判断 agent 看到某个帖子时，能否找回自己过去和这个帖子结构相关的历史动作。

为什么需要它：

- 当前 runtime query 通常只来自第一条可见 post/group 的 `summary`；
- `VAL-LTM-05` 的 query 从目标 episode 自身字段反推；
- 两者不是同一个问题；
- 如果只看真实 `last_recall_query_text`，又会遇到“第一条帖子 query 没有天然唯一正确答案”的问题；
- post-based replay 通过每个可见 post 的 `summary` 出题，再用结构化 `post_id` 关系定义正确答案，能在不向检索文本塞 id 的前提下得到更清晰的 ground truth。

当前实现：

- `step_audit.jsonl` 已补充每个 agent 的 `prompt_visible_snapshot`；
- `VAL-RCL-11 post_based_runtime_replay` 从 official step 中提取每个可见 post；
...
- `runtime_first_post_with_ground_truth_count`
- `non_first_post_with_ground_truth_count`
- `hit_at_1`
- `hit_at_3`
- `mrr`
- `runtime_first_post_hit_at_3`
- `non_first_post_hit_at_3`
...
- `runtime_first_post_*` 最贴近当前系统真实 runtime query，因为当前 runtime 通常只查第一个帖子；
- `non_first_post_*` 是诊断扩展，用来暴露“其他可见帖子也有可召回历史，但当前 runtime 没查”的覆盖缺陷；
- `post_probe_without_ground_truth_count` 不算 miss，它表示当前可见帖子在历史中没有结构化可测答案；
...
在 `B-level v0`、post-based runtime replay 和必要的 controlled benchmark 稳定之后，再补更完整固定 scenario packs：
...
- retrieval KPI 已稳定；
- injection 指标已可读；
- post-based runtime replay / 完整 query trace replay 已能解释真实查询质量；
- 已有足够真实运行日志用于设计判定规则。
