...
export function useSimulationStatus(pollingInterval: number = 3000) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (isMountedRef.current) {
        const error =
          err instanceof Error ? err : new Error(err?.message || 'Failed to fetch status')
        setError(error)
...
export function useMetricsSummary(pollingInterval: number = 5000) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (isMountedRef.current) {
        const error =
          err instanceof Error ? err : new Error(err?.message || 'Failed to fetch metrics')
        setError(error)
        ...
        if (isMounted) {
          const error =
            err instanceof Error
              ? err
              : new Error(err?.message || 'Failed to fetch propagation metrics')
          setError(error)
      ...
      try {
        ...
        if (isMounted) {
          const error =
            err instanceof Error
              ? err
              : new Error(err?.message || 'Failed to fetch polarization metrics')
          setError(error)
      ...
      try {
        ...
        if (isMounted) {
          const error =
            err instanceof Error
              ? err
              : new Error(err?.message || 'Failed to fetch herd effect metrics')
          setError(error)
...
export function useMetricsHistory(currentStep?: number) {
  ...
  return {
    ...
    error,
    refetch: () => {
      /* TODO: Implement refetch if needed */
    },
  }
}
