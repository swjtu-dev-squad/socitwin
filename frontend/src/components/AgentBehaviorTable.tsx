      ...
      <Table className="table-fixed">
        <TableHeader className="sticky top-0 bg-bg-secondary z-10">
          <TableRow className="border-border-default hover:bg-transparent">
            <TableHead className="h-10 px-3 text-text-tertiary font-bold uppercase text-[10px] tracking-widest">
              Agent
            </TableHead>
            <TableHead className="h-10 px-3 text-text-tertiary font-bold uppercase text-[10px] tracking-widest">
              角色
        ...
        <TableBody>
          {rows.map(agent => (
            ...
            >
              <TableCell className="h-12 px-3 py-2 font-bold text-sm truncate">
                {agent.name}
              </TableCell>
              ...
              <TableCell className="h-12 px-3 py-2 font-mono text-emerald-500">
...
function getAgentMemoryDisplay(memory: AgentOverview['memory'] | null | undefined) {
  ...
  return {
    ...
    title: ['本轮没有进入长期记忆召回阶段。', querySource ? `最近一次查询来源：${querySource}` : '']
      .filter(Boolean)
      .join('\n'),
    stageClassName: 'border-border-default bg-bg-tertiary text-text-tertiary',
  }
