...
import { User, Brain, History, Tag, MessageSquare, FileText, X } from 'lucide-react'
          ...
          >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {agent.interests.map(interest => (
                      <Badge
                        key={interest}
                        variant="secondary"
                        className="bg-bg-tertiary border-border-default text-[9px]"
                      >
                        {interest}
                      </Badge>
                    ))}
              ...
              <section className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 rounded-2xl bg-bg-primary/50 border border-border-default">
                    <p className="text-[10px] font-bold text-text-tertiary uppercase mb-1">
                      极化程度
                    </p>
                    <p className="text-2xl font-mono font-bold text-rose-500">
                      {(agent.polarization || 0).toFixed(4)}
                    </p>
                  </div>
                  <div className="p-4 rounded-2xl bg-bg-primary/50 border border-border-default">
                    <p className="text-[10px] font-bold text-text-tertiary uppercase mb-1">
            ...
            <div className="p-6 border-t border-border-default bg-bg-secondary/50 backdrop-blur-md grid grid-cols-2 gap-4">
              <Button
                ...
                onClick={() => navigate(`/logs?agent=${agent.id}`)}
              >
